...
// #define GxEPD2_DRIVER_CLASS GxEPD2_290_T94_V2     // GDEH029A1   128x296, SSD1608 (IL3820), (E029A01-FPC-A1 SYX1553)
...
void drawCustomContent();  // 新增这行，声明自定义函数
void helloWorld();
...
void setup()
{
  ...
  // ********************************************************* //
  // *** 修正HSPI初始化：显式配置SPI模式和速率 *** //
#if defined(ESP32) && defined(USE_HSPI_FOR_EPD)
  hspi.begin(14, -1, 13);  // sck=14, miso=-1, mosi=13（和接线一致）
  // 关键补充：设置SPI时序（E029A01默认用MODE0，部分用MODE3，可都试）
  SPISettings epd_spi_settings(4000000, MSBFIRST, SPI_MODE0); // 4MHz速率，大端模式，MODE0
  display.epd2.selectSPI(hspi, epd_spi_settings); // 传入配置好的SPI参数
#endif
  // *** 修正屏幕初始化：去掉多余的串口波特率（115200无用，用默认即可） *** //
  display.init(); // 原代码是display.init(115200)，改为无参，避免干扰
  Serial.println("屏幕初始化完成"); // 加打印，确认初始化执行
  // first update should be full refresh
//   helloWorld();
//   Serial.println("helloWorld执行完成"); // 加打印，确认显示函数执行
  drawCustomContent();  // 调用自定义内容绘制
  delay(5000);
   // 显示自定义位图
  display.setFullWindow(); // 全屏幕刷新（位图需要全刷）
  display.firstPage();     // 开始绘制循环
  do {
    display.fillScreen(GxEPD_WHITE); // 清空背景为白色
    // 调用自动生成的绘制函数（替换为你的图片对应的函数名）
    // 示例：如果转换的是test.jpg，函数名是draw_test_jpg
    draw_rgb_cam_1758806792_png();
  } while (display.nextPage()); // 完成绘制并刷新屏幕
    delay(5000);
...
void drawCustomContent() {
  // 1. 配置显示参数（旋转、字体、颜色）
  display.setRotation(1);  // 屏幕旋转：0=默认（竖屏）、1=右旋转90°（横屏，适合2.9英寸）
  display.setFont(&FreeMonoBold9pt7b);  // 字体（可替换为其他字体，如FreeSans12pt7b）
  display.setTextColor(GxEPD_BLACK);    // 文本颜色（GxEPD_WHITE/GxEPD_BLACK，三色屏可加GxEPD_RED）

  // 2. 定义自定义内容（文本、图形的坐标和参数）
  const char* customText1 = "我的电子纸";    // 第一行文本
  const char* customText2 = "2025-12-16";   // 第二行文本（可替换为动态数据，如传感器值）
  uint16_t screenW = display.width();       // 屏幕宽度（2.9英寸为296px，旋转后为296px）
  uint16_t screenH = display.height();      // 屏幕高度（2.9英寸为128px，旋转后为128px）

  // 3. 开始绘制（必须用 firstPage() + do-while 循环）
  display.setFullWindow();  // 全屏幕刷新（如果只想刷新局部，用 setPartialWindow()，见下文）
  display.firstPage();
  do {
    // 清空屏幕为白色（电子纸默认背景为白，每次绘制前建议清空）
    ...
    uint16_t y1 = screenH / 3 - tby1;           // 垂直位置（上1/3处）
